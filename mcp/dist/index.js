...
import { queryPartners, queryDeals, getAttributionsForDeal, getAttributionsForPartner, queryPayouts, getRevenueMetrics, getPartnerScore, getTopPartners, getAtRiskDeals, partners, deals, attributions, payouts, auditLog, } from "./data.js";
// ── Server Setup ──
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "query_partners": {
                const result = queryPartners(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            case "query_deals": {
                const result = queryDeals(args);
                return {
                    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
                };
            }
            ...
            case "query_payouts": {
                const result = queryPayouts(args);
