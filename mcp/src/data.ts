...
export function queryDeals(filters?: {
  ...
  if (filters?.minAmount != null) result = result.filter(d => d.amount >= filters.minAmount!);
  if (filters?.maxAmount != null) result = result.filter(d => d.amount <= filters.maxAmount!);
  return result;
}

export function getAttributionsForDeal(dealId: string): Attribution[] {
  return attributions.filter(a => a.dealId === dealId);
}

export function getAttributionsForPartner(partnerId: string): Attribution[] {
...
export function getPartnerScore(partnerId: string) {
...
export function getTopPartners(limit: number = 5) {
  return partners
...
export function getAtRiskDeals() {
  return deals.filter(d => {
    ...
    if (d.expectedCloseDate && d.expectedCloseDate < now + 14 * day) return true;
    return false;
  });
}
