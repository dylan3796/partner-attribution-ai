    ...
    return (
      <div style={{ flex: 1, minWidth: 280 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem", padding: "0 .5rem" }}>
          ...
          <span className="muted" style={{ fontSize: ".8rem" }}>{fmt(total)}</span>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: ".5rem" }}>
          ...
          {items.length === 0 && <p className="muted" style={{ textAlign: "center", padding: "2rem", fontSize: ".85rem" }}>No deals</p>}
        </div>
      </div>
    );
  }
      ...
      <div className="dash-content">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            ...
            <p className="muted">{deals.length} deals · {fmt(deals.filter((d) => d.status === "open").reduce((s, d) => s + d.amount, 0))} pipeline</p>
          </div>
          <div style={{ display: "flex", gap: ".5rem" }}>
            ...
            <button className="btn-outline" onClick={() => exportDealsCSV(deals)}><Download size={15} /> Export</button>
            <button className="btn" onClick={() => setShowAdd(true)}><Plus size={15} /> New Deal</button>
          </div>
        </div>

        {view === "pipeline" ? (
          <div style={{ display: "flex", gap: "1.5rem", overflow: "auto" }}>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowAdd(false)}>
            <div className="card" style={{ width: 480 }} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div><label className="muted" style={{ fontSize: ".8rem", display: "block", marginBottom: ".3rem" }}>Deal Name *</label><input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Enterprise CRM — Globex Corp" /></div>
                <div><label className="muted" style={{ fontSize: ".8rem", display: "block", marginBottom: ".3rem" }}>Amount *</label><input className="input" type="number" value={form.amount || ""} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} placeholder="120000" /></div>
