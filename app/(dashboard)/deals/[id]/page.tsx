...
export default function DealDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!deal) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/dashboard/deals" className="text-indigo-600 hover:underline text-sm mt-2 inline-block">
          Back to deals
        </Link>
      </div>
    );
  ...
  // Sorted touchpoints (timeline)
  const timeline = [...touchpoints].sort((a, b) => a.createdAt - b.createdAt);

  // Attribution comparison data for bar chart
  ...
  // Radar chart data — each model as a dimension, each partner as a series
  const radarData = MODELS.map((model) => {
    ...
    partnerIds.forEach((pid) => {
      ...
      const attr = allAttributions.find((a) => a.partnerId === pid && a.model === model);
      entry[partner?.name || pid] = attr?.percentage || 0;
    });
    return entry;
  });

  const statusIcon = deal.status === "won" ? (
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={String(involvedPartners.length)}
          subtitle={`${touchpoints.length} touchpoints`}
          icon={Users}
        />
        <StatCard
          title="Timeline"
          value={timeline.length > 0 ? `${Math.ceil((timeline[timeline.length - 1].createdAt - timeline[0].createdAt) / 86400000)} days` : "—"}
                ...
                <div key={tp._id} className="flex-1 min-w-[160px] relative px-2">
                  <div className="flex flex-col items-center">
                    ...
                    <div className="mt-3 text-center">
                      ...
                      {tp.notes && (
                        <p className="text-xs text-gray-400 mt-1 max-w-[140px] truncate">{tp.notes}</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <p className="text-xs text-gray-400 mb-6">
              How each model attributes credit to partners for this {formatCurrency(deal.amount)} deal
            </p>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={comparisonData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Legend
                    formatter={(value: string) => MODEL_LABELS[value as AttributionModel] || value}
                  />
                  {MODELS.map((model) => (
                    <Bar
                      key={model}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div
                                ...
                                style={{ backgroundColor: MODEL_COLORS[attr.model] }}
                              />
          ...
          {involvedPartners.map((partner) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all"
              >
                <Avatar name={partner.name} size="md" />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-gray-900 truncate">{partner.name}</p>
                  <p className="text-xs text-gray-400">
                    {pTouchpoints.length} touchpoints · {partner.commissionRate}% rate
                  </p>
                </div>
