    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          {tpCount > 0 && (
            <span className="text-xs text-indigo-600 font-medium">{tpCount} touchpoints</span>
          )}
        </div>
      </Link>
    );
          ...
          <div className="flex rounded-lg border border-gray-200 overflow-hidden">
            <button
              ...
              className={cn(
                ...
                view === "pipeline" ? "bg-indigo-50 text-indigo-600" : "text-gray-400 hover:text-gray-600"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-2 transition-colors",
                view === "table" ? "bg-indigo-50 text-indigo-600" : "text-gray-400 hover:text-gray-600"
              )}
            >
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-700">Open</h3>
                <span className="text-xs text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">{open.length}</span>
              </div>
              <span className="text-xs font-medium text-gray-500">
                {formatCurrency(open.reduce((s, d) => s + d.amount, 0))}
              </span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-700">Won</h3>
                <span className="text-xs text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">{won.length}</span>
              </div>
              <span className="text-xs font-medium text-emerald-600">
                {formatCurrency(won.reduce((s, d) => s + d.amount, 0))}
              </span>
            </div>
            <div className="space-y-3">
              {won.map((deal) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-700">Lost</h3>
                <span className="text-xs text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">{lost.length}</span>
              </div>
              <span className="text-xs font-medium text-red-500">
                {formatCurrency(lost.reduce((s, d) => s + d.amount, 0))}
              </span>
            </div>
            <div className="space-y-3">
              {lost.map((deal) => (
                ...
                {filtered.map((deal) => (
                  <tr key={deal._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <Link href={`/dashboard/deals/${deal._id}`} className="text-sm font-medium text-gray-900 hover:text-indigo-600">
                        {deal.name}
                      </Link>
                    </td>
      ...
      <Modal open={showAddModal} onClose={() => setShowAddModal(false)} title="Create Deal">
        <form onSubmit={handleAdd} className="space-y-4">
          <Input
            label="Deal Name"
            id="deal-name"
            placeholder="e.g. Enterprise Suite — Acme Corp"
            value={formName}
