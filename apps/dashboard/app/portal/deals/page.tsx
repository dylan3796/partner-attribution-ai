...
function PipelineBar({ deals }: { deals: DemoDeal[] }) {
  ...
  const segments = [
    {
      label: "Won",
      value: won.reduce((s, d) => s + d.amount, 0),
      color: "#22c55e",
    },
    {
      label: "Open",
      value: open.reduce((s, d) => s + d.amount, 0),
      color: "#3b82f6",
    },
    {
      label: "Lost",
      value: lost.reduce((s, d) => s + d.amount, 0),
      color: "#ef4444",
    },
  ];
...
export default function PortalDealsPage() {
  ...
  const deals = useMemo<DemoDeal[]>(() => {
    ...
    return source.map((deal: any) => ({
      ...
      registrationStatus: (deal.registrationStatus || "approved") as
        | "pending"
        | "approved"
        | "rejected",
  ...
  const wonRevenue = deals
    ...
    .reduce((s, d) => s + d.amount, 0);
  const pendingReg = deals.filter(
    (d) => d.registrationStatus === "pending",
  ).length;
    ...
    try {
      if (session?.partnerId && convexPartner) {
        await registerDeal({
          ...
          expectedCloseDate: regForm.expectedCloseDate
            ? new Date(regForm.expectedCloseDate).getTime()
            : undefined,
    ...
    <>
      ...
      >
        <div>
          ...
          >
            My Deal Registrations
          </h1>
          <p className="muted">
            {deals.length} deals · {fmt(openPipeline)} pipeline
          </p>
        </div>
        <button
          className="btn"
              ...
              >
                ...
                >
                  ...
                  {deal.expectedCloseDate && (
                    <span className="muted">
                      <Calendar size={12} style={{ verticalAlign: -2 }} />{" "}
                      Close:{" "}
                      {new Date(deal.expectedCloseDate).toLocaleDateString()}
                    </span>
                  )}
                  <span className="muted">
                    Registered {new Date(deal.createdAt).toLocaleDateString()}
                  </span>
                </div>
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      {deal.approvedAt && (
                        <div style={{ fontSize: ".85rem", color: "#22c55e" }}>
                          Approved:{" "}
                          {new Date(deal.approvedAt).toLocaleDateString()}
                        </div>
                      )}
                      {deal.expectedCloseDate && (
                        <div style={{ fontSize: ".85rem" }}>
            ...
            {submitted ? (
              <div style={{ textAlign: "center", padding: "2rem 0" }}>
                ...
                <p className="muted" style={{ marginBottom: 4 }}>
                  Your deal registration is pending approval.
                </p>
                <p className="muted" style={{ fontSize: ".8rem" }}>
                  You'll be notified when it's reviewed (typically within 24h).
                </p>
                <button
                  className="btn"
              ...
              <>
                ...
                >
                  <div>
                    ...
                    <p className="muted" style={{ fontSize: ".8rem" }}>
                      Submit an opportunity for approval and tracking
                    </p>
                  </div>
                  <button
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setRegForm({ ...regForm, companyName: e.target.value })
                      }
                      style={{ width: "100%" }}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      className="input"
                      type="date"
                      value={regForm.expectedCloseDate}
                      onChange={(e) =>
                        setRegForm({
                          ...regForm,
                  ...
                  <div>
                    ...
                    <textarea
                      className="input"
                      rows={3}
                      placeholder="How did you find this opportunity? Context helps with approval..."
