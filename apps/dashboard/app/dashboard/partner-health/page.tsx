...
function riskBadge(risk: RiskLevel): {
  ...
  switch (risk) {
    case "healthy":
      return { bg: "#22c55e18", color: "#22c55e", label: "Healthy" };
    case "at-risk":
      return { bg: "#f59e0b18", color: "#f59e0b", label: "At Risk" };
    case "churning":
      return { bg: "#ef444418", color: "#ef4444", label: "Churn Risk" };
    case "new":
      return { bg: "#3b82f618", color: "#3b82f6", label: "New" };
  }
}
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div
                className="skeleton"
                style={{ height: 18, width: 160, marginBottom: 6 }}
              />
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        >
          Individual partner health monitoring — engagement, revenue, risk
      ...
      >
        ...
        />
        <h2 style={{ fontSize: "1.1rem", fontWeight: 700, margin: "0 0 6px" }}>
        ...
        >
          Add partners and register deals to start seeing health scores, risk
...
export default function PartnerHealthPage() {
  ...
  const partners: PartnerHealthData[] = (healthData ??
    []) as unknown as PartnerHealthData[];
    ...
    list.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "name":
          ...
          break;
        case "revenue":
          cmp = a.revenueLast90 - b.revenueLast90;
          break;
        case "lastActive":
          cmp = a.daysSinceActive - b.daysSinceActive;
          break;
        case "deals":
          cmp = a.dealsLast90 - b.dealsLast90;
          break;
      }
  ...
  const avgHealth = Math.round(
    partners.reduce((s, p) => s + p.healthScore, 0) / partners.length,
  );
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      >
        {[
          ...
          <div key={i} className="card" style={{ padding: "1.25rem" }}>
            ...
            <div
              style={{ fontSize: "1.5rem", fontWeight: 800, color: card.color }}
            >
              {card.value}
            </div>
            {card.sub && (
        ...
        >
          ...
          <span style={{ fontSize: ".82rem" }}>
            <strong style={{ color: "#ef4444" }}>
              {churningCount} partner{churningCount > 1 ? "s" : ""} at churn
              risk
            </strong>{" "}
            — immediate action recommended. Sort by health score to prioritize.
          </span>
        </div>
      )}
      ...
      >
        ...
        {(
          ["all", "healthy", "at-risk", "churning", "new"] as (
            | RiskLevel
            | "all"
          )[]
        ).map((f) => {
          const count =
            f === "all"
              ? partners.length
              : partners.filter((p) => p.risk === f).length;
          return (
            <button
              key={f}
            ...
            >
              ...
              >
                ...
                >
                  <svg width={48} height={48} viewBox="0 0 48 48">
                    ...
                    <circle
                      ...
                      r={20}
                      fill="none"
                      stroke={healthColor(p.healthScore)}
                      strokeWidth={4}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {p.dealsLast90} deals · {fmt(p.revenueLast90)} revenue (90d)
                    · Active{" "}
                    {p.daysSinceActive === 0
                      ? "today"
                      : `${p.daysSinceActive}d ago`}
                  </div>
                </div>
                  ...
                  {p.revenueTrend !== 0 && (
                    <span
                      style={{
                        ...
                        fontSize: ".78rem",
                        fontWeight: 700,
                        color: p.revenueTrend > 0 ? "#22c55e" : "#ef4444",
                      }}
                    >
                      {p.revenueTrend > 0 ? (
                        <TrendingUp size={14} />
                ...
                >
                  ...
                  >
                    {[
                      {
                        label: "Deals (90d)",
                        value: p.dealsLast90.toString(),
                        trend: p.dealsTrend,
                      },
                      {
                        label: "Revenue (90d)",
                        value: fmt(p.revenueLast90),
                        trend: p.revenueTrend,
                      },
                      {
                        label: "Touchpoints (90d)",
                        value: p.touchpointsLast90.toString(),
                        trend: null,
                      },
                      {
                        label: "Unpaid Commissions",
                        value: p.unpaidCommissions.toString(),
                        trend: null,
                      },
                      {
                        label: "Days Since Active",
                        value: p.daysSinceActive.toString(),
                        trend: null,
                      },
                    ].map((m, i) => (
                        ...
                        {m.trend !== null && m.trend !== 0 && (
                          <div
                            style={{
                              fontSize: ".65rem",
                              fontWeight: 600,
                              color: m.trend > 0 ? "#22c55e" : "#ef4444",
                            }}
                          >
                            {m.trend > 0 ? "+" : ""}
                            {m.trend} vs prior 90d
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        borderRadius: 10,
                        background:
                          p.risk === "churning" ? "#ef444408" : "var(--subtle)",
                      }}
                    >
                      <h4
                        style={{
                      ...
                      {p.actions.map((a, i) => (
                        ...
                        >
                          <span style={{ color: "#6366f1", marginTop: 2 }}>
                            →
                          </span>
                          {a}
                        </div>
