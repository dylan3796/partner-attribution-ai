  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        <span
          style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--bg)" }}
        >
          {value}
        </span>
...
export default function ScorecardPage({
  ...
  const metrics = useMemo(() => {
    ...
    const openDeals = deals.filter(
      (d: any) => !["closed_won", "closed_lost"].includes(d.status),
    );
    ...
    const winRate =
      totalDeals > 0
        ? Math.round(
            (wonDeals.length / (wonDeals.length + lostDeals.length || 1)) * 100,
          )
        : 0;
    ...
    const pendingCommissions = payouts
      .filter((p: any) => p.status !== "paid")
      .reduce((sum: number, p: any) => sum + (p.amount || 0), 0);
    ...
    const prevDeals = deals.filter(
      (d: any) => d.createdAt > now - MS_90D * 2 && d.createdAt <= now - MS_90D,
    );
    const recentRevenue = wonDeals
      .filter((d: any) => (d.closedAt || d.createdAt) > now - MS_90D)
      .reduce((sum: number, d: any) => sum + (d.amount || 0), 0);
    const prevRevenue = wonDeals
      .filter(
        (d: any) =>
          (d.closedAt || d.createdAt) > now - MS_90D * 2 &&
          (d.closedAt || d.createdAt) <= now - MS_90D,
      )
      .reduce((sum: number, d: any) => sum + (d.amount || 0), 0);
    ...
    const dealsTrend =
      prevDeals.length > 0
        ? Math.round(
            ((recentDeals.length - prevDeals.length) / prevDeals.length) * 100,
          )
        : 0;
    ...
    const daysSinceActivity =
      lastActivity > 0
        ? Math.round((now - lastActivity) / (24 * 60 * 60 * 1000))
        : 999;
    ...
    return (
      ...
      >
        ...
        <Link
          ...
          style={{ color: "#6366f1", fontSize: ".875rem" }}
        >
          ← Back to Partners
        </Link>
      </div>
    ...
    <>
      ...
      >
          ...
          <div>
            ...
            >
              ...
              >
                <Building2 size={13} />{" "}
                {PARTNER_TYPE_LABELS[
                  partner.type as keyof typeof PARTNER_TYPE_LABELS
                ] || partner.type}
              </span>
              {partner.email && (
                <span
        ...
        >
          ...
          <div>
            <div
              style={{ fontSize: "1rem", fontWeight: 600, color: "var(--bg)" }}
            >
              Health Score:{" "}
              <span style={{ color: healthColor(metrics.healthScore) }}>
                {healthLabel(metrics.healthScore)}
              </span>
            </div>
            <div
              style={{ fontSize: ".8rem", color: "var(--muted)", marginTop: 2 }}
            >
        ...
        >
          <MetricCard
            ...
            sub={`${metrics.wonDeals} closed-won deals`}
          />
          <MetricCard
            label="Open Pipeline"
            value={formatCurrency(metrics.openPipeline)}
            icon={Target}
            sub={`${metrics.openDeals} active deals`}
          />
          <MetricCard
            label="Win Rate"
            value={`${metrics.winRate}%`}
            icon={Award}
            sub={`${metrics.wonDeals}W / ${metrics.lostDeals}L`}
          />
          <MetricCard
            label="Avg Deal Size"
            ...
            sub={
              metrics.avgVelocity > 0
                ? `${metrics.avgVelocity}d avg velocity`
                : undefined
            }
          />
        </div>
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {deal.name || deal.dealName || "Untitled Deal"}
                      </div>
                      {deal.productName && (
                        <div
          ...
          >
            {metrics.totalRevenue > 0 && metrics.totalCommissions > 0 && (
              <p style={{ margin: 0 }}>
                ...
                <strong style={{ color: "var(--bg)" }}>
                  {(
                    (metrics.totalCommissions / metrics.totalRevenue) *
                    100
                  ).toFixed(1)}
                  %
                </strong>
            ...
            {metrics.revenueTrend > 0 && (
              <p style={{ margin: 0, color: "#22c55e" }}>
                📈 Revenue is trending{" "}
                <strong>up {metrics.revenueTrend}%</strong> vs prior 90 days
              </p>
            )}
            {metrics.revenueTrend < 0 && (
              <p style={{ margin: 0, color: "#ef4444" }}>
                📉 Revenue is trending{" "}
                <strong>down {Math.abs(metrics.revenueTrend)}%</strong> vs prior
                90 days — may need attention
              </p>
            )}
            {metrics.winRate >= 60 && (
              <p style={{ margin: 0 }}>
                🎯 Win rate of{" "}
                <strong style={{ color: "#22c55e" }}>{metrics.winRate}%</strong>{" "}
                is above average — strong deal execution
              </p>
            )}
            {metrics.winRate > 0 && metrics.winRate < 30 && (
              <p style={{ margin: 0 }}>
                ⚠️ Win rate of{" "}
                <strong style={{ color: "#eab308" }}>{metrics.winRate}%</strong>{" "}
                — consider enablement or deal qualification review
              </p>
            )}
            {metrics.avgVelocity > 0 && (
              <p style={{ margin: 0 }}>
                ⏱️ Average deal closes in{" "}
                <strong style={{ color: "var(--bg)" }}>
                  {metrics.avgVelocity} days
                </strong>
              </p>
            )}
            {metrics.pendingCommissions > 0 && (
              <p style={{ margin: 0 }}>
                ...
                in commissions awaiting approval
              </p>
            )}
            {metrics.openPipeline > 0 && (
              <p style={{ margin: 0 }}>
                ...
                in open pipeline across {metrics.openDeals} deals
              </p>
            )}
            {metrics.daysSinceActivity > 60 &&
              metrics.daysSinceActivity < 999 && (
                <p style={{ margin: 0, color: "#ef4444" }}>
                  🚨 No activity in{" "}
                  <strong>{metrics.daysSinceActivity} days</strong> — partner
        ...
        >
          <span style={{ fontSize: ".7rem", color: "#6b7280" }}>
            Generated by Covant · {generatedDate}
          </span>
