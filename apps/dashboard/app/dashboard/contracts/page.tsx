...
export default function ContractsPage() {
  ...
  const [sortBy, setSortBy] = useState<"expiry" | "value" | "partner">(
    "expiry",
  );
    ...
    return contracts
      .filter((c) => {
        ...
        if (search) {
          ...
          return (
            (c.partnerName as string).toLowerCase().includes(q) ||
      ...
      .sort((a, b) => {
        ...
        return (
          ((a.partnerName as string) || "").localeCompare(
            (b.partnerName as string) || "",
          ) * dir
        );
      });
  ...
  const stats = useMemo(
    () => ({
      ...
      expiringSoon: contracts.filter(
        (c) => c.status === "expiring_soon" || c.status === "pending_renewal",
      ).length,
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((c) => [
      c.partnerName,
      c.title,
      TYPE_LABELS[c.type as ContractType],
      STATUS_CONFIG[c.status as ContractStatus]?.label,
      c.value,
      c.commissionRate,
      c.territory || "",
      c.startDate || "",
      ...
      >
        <div>
          ...
          <p className="muted" style={{ marginTop: ".25rem" }}>
            Manage partner contracts, renewals, and compliance
          </p>
        </div>
        ...
        >
          ...
          <div>
            <p style={{ fontWeight: 600, fontSize: ".9rem", color: "#f59e0b" }}>
              ...
              need{stats.expiringSoon === 1 ? "s" : ""} attention
            </p>
          ...
          >
            ...
            <tbody>
              {filtered.map((c) => {
                ...
                const tierColor =
                  TIER_COLORS[(c.partnerTier as string)?.toLowerCase()] ||
                  "#64748b";
                return (
                  <tr
                    key={c._id}
                      ...
                      {c.endDate ? (
                        <div>
                          ...
                          {days !== null && days > 0 && days <= 30 && (
                            <div
                              style={{ fontSize: ".7rem", color: "#f59e0b" }}
                            >
                              {days}d remaining
                            </div>
                          )}
                          {days !== null && days < 0 && (
                            <div
                              style={{ fontSize: ".7rem", color: "#ef4444" }}
                            >
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <div
                    style={{ color: c.autoRenew ? "#22c55e" : "var(--muted)" }}
                  >
                    {c.autoRenew ? "Yes" : "No"}
                  </div>
                </div>
