  ...
  async function handleReject() {
    ...
    try {
      await rejectMutation({
        id: showRejectModal as Id<"payouts">,
    ...
    <>
      ...
      >
        <div>
          ...
          <p className="muted">
            Manage partner commissions and payment workflows
          </p>
        </div>
        ...
        <div className="card" style={{ padding: "1rem 1.2rem" }}>
          ...
          >
            <div>
              <p
                className="muted"
                style={{ fontSize: ".75rem", marginBottom: ".2rem" }}
              >
                Pending Approval
              </p>
              <p
          ...
          <p
            className="muted"
            style={{ fontSize: ".75rem", marginTop: ".3rem" }}
          >
            {stats.pendingCount} payout{stats.pendingCount !== 1 ? "s" : ""}{" "}
            awaiting review
          </p>
        </div>
        <div className="card" style={{ padding: "1rem 1.2rem" }}>
          ...
          <p
            ...
            style={{ fontSize: ".75rem", marginTop: ".3rem" }}
          >
            {stats.approvedCount} ready to pay
          </p>
        </div>
        <div className="card" style={{ padding: "1rem 1.2rem" }}>
          <div
      ...
      {stats.pendingCount > 0 && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: ".75rem" }}>
            ...
            <div>
              <p
                style={{ fontWeight: 600, fontSize: ".9rem", color: "#92400e" }}
              >
                {stats.pendingCount} payout{stats.pendingCount !== 1 ? "s" : ""}{" "}
                pending approval
              </p>
              <p style={{ fontSize: ".8rem", color: "#a16207" }}>
                {fmtFull(stats.pendingAmount)} total awaiting review
              </p>
            </div>
          </div>
          <button
      ...
      {selected.size > 0 && (
        ...
        >
          <span style={{ fontSize: "0.9rem", fontWeight: 600 }}>
            {selected.size} payout{selected.size > 1 ? "s" : ""} selected
          </span>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        >
            ...
            >
              <th style={{ padding: ".8rem .5rem .8rem 1.2rem", width: 32 }}>
                <input
                  type="checkbox"
                  checked={
                    filtered.filter((p) => p.status === "pending_approval")
                      .length > 0 &&
                    filtered
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.checked) {
                      setSelected(
                        new Set([...selected, ...pending.map((p) => p._id)]),
                      );
                    } else {
          ...
          <tbody>
            {filtered.map((payout) => {
              const meta =
                STATUS_META[payout.status] || STATUS_META.pending_approval;
              ...
              return (
                ...
                >
                  <td
                    style={{ padding: ".8rem .5rem .8rem 1.2rem", width: 32 }}
                  >
                  ...
                  <td style={{ padding: ".8rem 1.2rem" }}>
                    ...
                    >
                      <div
                        className="avatar"
                        style={{ width: 32, height: 32, fontSize: ".65rem" }}
                      >
                        {(payout.partner?.name || "?")
                          .split(" ")
                          .map((w: string) => w[0])
                      ...
                      <div>
                        <Link
                          href={`/dashboard/partners/${payout.partnerId}`}
                    ...
                    >
                      ...
                      {payout.status === "approved" && (
                        <button
                          ...
                          onClick={() =>
                            setConfirmAction({ id: payout._id, action: "pay" })
                          }
                        >
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "3rem" }}>
            ...
            <p className="muted" style={{ fontSize: ".9rem" }}>
              {payouts.length === 0 ? (
                <>
                  No payouts yet. Payouts are created automatically when deals
                  close and commissions are calculated.
                </>
              ) : (
          ...
          >
            ...
            >
              ...
              <button
                ...
                disabled={
                  !form.partnerId || !form.amount || !form.period || saving
                }
              >
              ...
              >
                Payouts are created in &quot;Pending Approval&quot; status.
              </p>
            </div>
          </div>
        </div>
            ...
            {confirmAction.action === "approve" ? (
              <>
                ...
                <p className="muted" style={{ marginBottom: "1.5rem" }}>
                  This will approve the payout for{" "}
                  <strong>
                    {fmtFull(
                      payouts.find((p) => p._id === confirmAction.id)?.amount ||
                        0,
                    )}
                  </strong>{" "}
                  to{" "}
                  <strong>
                    {
                      payouts.find((p) => p._id === confirmAction.id)?.partner
                        ?.name
                    }
                  </strong>
                  .
        ...
        >
          ...
          >
            ...
            >
              ...
              <strong>
                {fmtFull(
                  payouts.find((p) => p._id === showRejectModal)?.amount || 0,
                )}
              </strong>{" "}
              to{" "}
              <strong>
                {payouts.find((p) => p._id === showRejectModal)?.partner?.name}
              </strong>
              .
            </p>
