      ...
      >
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="card"
            style={{ padding: "1.25rem", display: "flex", gap: "1rem" }}
          >
            <div
              style={{
                width: 44,
...
function EmptyState() {
  return (
    <div className="card" style={{ padding: "4rem 2rem", textAlign: "center" }}>
      ...
      >
        Partner incentive eligibility is calculated based on deal performance.
        Add partners and deals to see incentive tracking.
      </p>
...
const INCENTIVE_PROGRAMS = [
  ...
  },
  {
    ...
    type: "accelerator",
    description:
      "5+ deals AND $200k+ revenue unlocks 1.5x commission multiplier",
...
export default function IncentivesPage() {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        <div>
          ...
          <p className="muted" style={{ marginTop: "0.25rem" }}>
            Partner performance and incentive eligibility
          </p>
        </div>
        <EmptyState />
      </div>
  ...
  const eligibleForSpif = partnerPerformance.filter(
    (p: any) => p.incentives.spifEligible,
  ).length;
  const eligibleForAccelerator = partnerPerformance.filter(
    (p: any) => p.incentives.acceleratorEligible,
  ).length;
  const totalCommissions = partnerPerformance.reduce(
    (s: number, p: any) => s + p.metrics.totalCommission,
    0,
  );
  const totalRevenue = partnerPerformance.reduce(
    (s: number, p: any) => s + p.metrics.totalRevenue,
    0,
  );
  ...
  const filtered =
    filterType === "all"
      ...
      : partnerPerformance.filter((p: any) => {
          ...
          if (filterType === "accelerator")
            return p.incentives.acceleratorEligible;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      >
        <div>
          ...
          <p className="muted" style={{ marginTop: "0.25rem" }}>
            Partner performance and incentive eligibility based on real deal
            data
          </p>
        </div>
        <select
      ...
      >
        <StatCard
          icon={<Zap size={22} />}
          label="SPIF Eligible"
          value={String(eligibleForSpif)}
          sub={`of ${partnerPerformance.length} partners`}
        />
        <StatCard
          icon={<DollarSign size={22} />}
          label="Total Revenue"
          value={formatCurrency(totalRevenue)}
          sub={`from ${partnerPerformance.length} partners`}
        />
        <StatCard
          icon={<Award size={22} />}
          label="Top Performers"
          value={String(eligibleForAccelerator)}
          sub="accelerator eligible"
        />
        <StatCard
          icon={<Gift size={22} />}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {program.rewardType === "multiplier" &&
                      `${program.reward}x multiplier`}
                  </span>
                </div>
              </div>
            );
          ...
          {filtered.map((partner: any, idx: number) => {
            ...
            return (
              <div key={partner._id}>
                <div
                  onClick={() =>
                    setSelectedPartnerId(isSelected ? null : partner._id)
                  }
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      <IncentiveBadge
                        type="spif"
                        eligible={partner.incentives.spifEligible}
                      />
                      <IncentiveBadge
                        type="bonus"
                        eligible={partner.incentives.bonusEligible}
                      />
                      <IncentiveBadge
                        type="accelerator"
                        eligible={partner.incentives.acceleratorEligible}
                      />
                      <IncentiveBadge
                        type="dealReg"
                        eligible={partner.incentives.dealRegBonus}
                      />
                    </div>
                  </div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div className="muted" style={{ fontSize: ".8rem" }}>
                      {partner.metrics.dealsWon} won ·{" "}
                      {partner.metrics.dealsOpen} open
                    </div>
                  </div>
                </div>
                  ...
                  >
                    ...
                    >
                      <div>
                        <p
                          className="muted"
                          style={{ fontSize: ".75rem", marginBottom: 4 }}
                        >
                          Total Revenue
                        </p>
                        <p
                      ...
                      <div>
                        ...
                        >
                          {formatCurrency(partner.metrics.openPipeline)}
                        </p>
                      </div>
                      <div>
                        <p
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: "1.2rem", fontWeight: 800 }}>
                          {formatCurrency(partner.metrics.avgDealSize)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: "1.25rem" }}>
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            ...
                            <span style={{ fontWeight: 600 }}>
                              {partner.metrics.dealsWon}/3
                            </span>
                          </div>
                          <ProgressBar
                            value={partner.metrics.dealsWon}
                            max={3}
                            color={
                              partner.incentives.spifEligible
                                ? "#22c55e"
                                : "#6366f1"
                            }
                          />
                        ...
                        <div>
                          ...
                          <ProgressBar
                            value={partner.metrics.totalRevenue}
                            max={100000}
                            color={
                              partner.incentives.bonusEligible
                                ? "#22c55e"
                                : "#6366f1"
                            }
                          />
                        ...
                        <div>
                          ...
                          >
                            ...
                            <span style={{ fontWeight: 600 }}>
                              {partner.metrics.dealsRegistered}/2
                            </span>
                          </div>
                          <ProgressBar
                            value={partner.metrics.dealsRegistered}
                            max={2}
                            color={
                              partner.incentives.dealRegBonus
                                ? "#22c55e"
                                : "#6366f1"
                            }
                          />
