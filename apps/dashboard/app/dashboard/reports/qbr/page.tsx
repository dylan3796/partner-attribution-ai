...
function DeltaBadge({
  ...
  if (previous === 0 && current === 0)
    return <span style={{ fontSize: ".75rem", color: "var(--muted)" }}>—</span>;
  const pct =
...
function QBRContent({
  ...
  const currentQRevenue =
    rev.slice(-3).reduce((a: number, b: number) => a + b, 0) ||
    stats.totalRevenue;
  const prevQRevenue =
    rev.slice(-6, -3).reduce((a: number, b: number) => a + b, 0) || 0;
  const currentQPipeline =
    pip.length > 0 ? pip[pip.length - 1] : stats.pipelineValue;
  const prevQPipeline = pip.length >= 4 ? pip[pip.length - 4] : 0;
  const currentQPartners =
    prt.length > 0 ? prt[prt.length - 1] : stats.totalPartners;
  ...
  const quarterProgress = Math.min(
    100,
    Math.round(
      ((now.getTime() - currentQ.start.getTime()) /
        (currentQ.end.getTime() - currentQ.start.getTime())) *
        100,
    ),
  );
    ...
    >
      ...
      >
        <div>
          ...
          <p className="muted" style={{ fontSize: ".85rem" }}>
            Executive summary of partner program performance · Generated{" "}
      ...
      <div className="card" style={{ padding: "1rem 1.25rem" }}>
        ...
        >
          <span className="muted" style={{ fontSize: ".7rem" }}>
            {currentQ.start.toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
            })}
          </span>
          <span className="muted" style={{ fontSize: ".7rem" }}>
            {currentQ.end.toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
            })}
          </span>
      ...
      <div>
        <SectionHeader
          ...
          subtitle="Key metrics vs. previous quarter"
        />
        <div
          style={{
            display: "grid",
        ...
        >
          <StatCard
            ...
            iconColor="#065f46"
            delta={
              <DeltaBadge current={currentQRevenue} previous={prevQRevenue} />
            }
          ...
          <StatCard
            ...
            iconColor="#3730a3"
            delta={
              <DeltaBadge current={currentQPipeline} previous={prevQPipeline} />
            }
          ...
          <StatCard
            ...
            iconColor="#166534"
            delta={
              <DeltaBadge current={currentQPartners} previous={prevQPartners} />
            }
          ...
          <StatCard
            ...
            iconColor="#92400e"
            delta={
              <DeltaBadge current={currentQWinRate} previous={prevQWinRate} />
            }
      ...
      >
        ...
        <div>
          <SectionHeader
            ...
            subtitle="Spend and outstanding payouts"
          />
          <div className="card" style={{ padding: "1.25rem" }}>
            <div
              style={{
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {formatCurrencyCompact(pendingPayouts)}
                  </p>
                </div>
                <div>
                  <p
                  ...
                  >
                    {formatCurrencyCompact(totalCommissions - pendingPayouts)}
                  </p>
                </div>
              </div>
              <div style={{ height: 1, background: "var(--border)" }} />
              <div>
                ...
                >
                  ...
                  >
                    {currentQRevenue > 0
                      ? `${((totalCommissions / currentQRevenue) * 100).toFixed(1)}%`
                      : "—"}
                  </span>
                </div>
              </div>
                ...
                >
                  ...
                  <td style={tdStyle}>
                    ...
                    >
                      <div
                        className="avatar"
                        style={{ width: 28, height: 28, fontSize: ".65rem" }}
                      >
                        {(p.name || "?")
                          .split(" ")
                          .map((w: string) => w[0])
                  ...
                  <td style={tdStyle}>
                    ...
                    >
                      {(p.tier || "standard").charAt(0).toUpperCase() +
                        (p.tier || "standard").slice(1)}
                    </span>
                  </td>
                  <td
      ...
      <div>
        <SectionHeader
          ...
          subtitle="Items requiring attention this quarter"
        />
        <div
          style={{
            display: "grid",
        ...
        >
          {actionItems ? (
            <>
              <ActionCard
                icon="📋"
                label="Pending Deal Registrations"
                value={actionItems.pendingDealRegs}
                status={actionItems.pendingDealRegs > 3 ? "warning" : "healthy"}
                href="/dashboard/deals"
              />
              <ActionCard
                icon="💰"
                label="Unpaid Commissions"
                value={formatCurrencyCompact(actionItems.unpaidCommissions)}
                status={
                  actionItems.unpaidCommissions > 10000 ? "warning" : "healthy"
                }
                href="/dashboard/payouts"
              />
              <ActionCard
                icon="🏆"
                label="Tier Reviews Due"
                value={actionItems.tierReviewsPending}
                status={
                  actionItems.tierReviewsPending > 0 ? "warning" : "healthy"
                }
              ...
              <ActionCard
                ...
                value={actionItems.partnersOnboarding}
                status="info"
              ...
              <ActionCard
                ...
                value={actionItems.pendingInvites}
                status={actionItems.pendingInvites > 5 ? "warning" : "healthy"}
                href="/dashboard/partners"
              />
              <ActionCard
                icon="📧"
                label="Email Triggers"
                value={`${actionItems.emailTriggersActive}/${actionItems.emailTriggersTotal}`}
                status="info"
  ...
  return (
    <Link href={href} style={{ textDecoration: "none" }}>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div
            style={{ fontSize: ".9rem", fontWeight: 700, color: statusColor }}
          >
            {value}
          </div>
