...
function TierBadge({
  ...
  const s =
    size === "sm"
      ...
      : { padding: "3px 12px", fontSize: ".8rem" };
  return (
    <span
      style={{
        ...s,
...
export default function TierReviewsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "upgrades" | "downgrades">(
    "all",
  );
  ...
  const [, setNoteTimers] = useState<Map<string, NodeJS.Timeout>>(new Map());
  ...
  const changesOnly = useMemo(
    () => scores.filter((s) => s.tierChange !== "maintain"),
    [scores],
  );
  ...
  const handleAction = useCallback(
    async (score: PartnerScore, action: ReviewAction) => {
      ...
      try {
        await saveReview({
          ...
          notes:
            localNotes.get(score.partnerId) ||
            reviewMap.get(score.partnerId)?.notes ||
            undefined,
        });
      } catch (err) {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
        <div>
          ...
          <p className="muted" style={{ marginTop: "0.25rem" }}>
            Review and approve partner tier changes based on scoring
          </p>
        </div>
        <div
          style={{
        ...
        >
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="card"
              style={{ padding: "1.25rem", height: 80 }}
            >
              <div
                style={{
                  width: "60%",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      {filtered.length === 0 ? (
        <div className="card" style={{ padding: "3rem", textAlign: "center" }}>
          ...
          <p className="muted" style={{ fontSize: ".875rem" }}>
            All partners are correctly tiered based on current scores
          </p>
        </div>
      ) : (
        <div
        ...
        >
          {filtered
            .sort((a, b) => {
              if (a.tierChange !== b.tierChange)
                return a.tierChange === "upgrade" ? -1 : 1;
              ...
              return (
                <div key={score.partnerId}>
                  <div
                    className="card"
                    onClick={() =>
                      setExpandedId(isExpanded ? null : score.partnerId)
                    }
                    style={{
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        style={{ display: "flex", gap: 12, fontSize: ".75rem" }}
                      >
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <ActionButton
                          ...
                          active={review?.action === "approved"}
                          onClick={() => handleAction(score, "approved")}
                          color="#22c55e"
                          saving={isSaving}
                        />
                        <ActionButton
                          ...
                          active={review?.action === "rejected"}
                          onClick={() => handleAction(score, "rejected")}
                          color="#ef4444"
                          saving={isSaving}
                        />
                        <ActionButton
                          label="Defer"
                          icon={<Clock size={14} />}
                          active={review?.action === "deferred"}
                          onClick={() => handleAction(score, "deferred")}
                          color="#eab308"
                          saving={isSaving}
                        />
                      </div>
                    ...
                    >
                        ...
                        {Object.entries(score.dimensions).map(([key, dim]) =>
                          dim ? (
                            ...
                            >
                              ...
                              <div
                                className="muted"
                                style={{ fontSize: ".75rem", lineHeight: 1.4 }}
                              >
                                {dim.detail}
                              </div>
                              <div
                        ...
                        <div>
                          ...
                          {score.highlights.map((h, i) => (
                            ...
                            >
                              <ChevronRight
                                size={14}
                                style={{ color: changeColor, flexShrink: 0 }}
                              />
                              <span>{h}</span>
                            </div>
                          ))}
                      ...
                      <div style={{ marginTop: 16 }}>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            handleNotesChange(score.partnerId, e.target.value)
                          }
                        ...
                        {review && (
                          <p
                            ...
                            style={{ fontSize: ".7rem", marginTop: 4 }}
                          >
                            Notes auto-save when you have an active review
                            decision
                          </p>
                        )}
                      </div>
      ...
      {changesOnly.length > 0 && (
        ...
        >
          <span className="muted" style={{ fontSize: ".875rem" }}>
            ...
            {reviewed > 0 &&
              (() => {
                const approved = changesOnly.filter(
                  (s) => reviewMap.get(s.partnerId)?.action === "approved",
                ).length;
                const rejected = changesOnly.filter(
                  (s) => reviewMap.get(s.partnerId)?.action === "rejected",
                ).length;
                const deferred = changesOnly.filter(
                  (s) => reviewMap.get(s.partnerId)?.action === "deferred",
                ).length;
