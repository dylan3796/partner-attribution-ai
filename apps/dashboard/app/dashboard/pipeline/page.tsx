...
function EmptyState() {
  return (
    <div className="card" style={{ padding: "4rem 2rem", textAlign: "center" }}>
      ...
      >
        Your partner-influenced pipeline will appear here once deals with
        partner touchpoints are created.
      </p>
...
function DealCard({
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: compact ? "1rem" : "1.1rem", fontWeight: 800 }}
        >
        ...
        <div style={{ marginBottom: 6 }}>
          <span
            style={{
              ...
              fontWeight: 700,
              background:
                deal.registrationStatus === "approved"
                  ? "#22c55e18"
                  : deal.registrationStatus === "rejected"
                    ? "#ef444418"
                    : "#eab30818",
              color:
                deal.registrationStatus === "approved"
        ...
        >
          {deal.involvedPartners && deal.involvedPartners.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <p
                className="muted"
                style={{ fontSize: ".65rem", fontWeight: 600, marginBottom: 4 }}
              >
                PARTNERS
              </p>
        ...
        {col.deals.length === 0 ? (
          ...
          col.deals.map((deal: any) => (
            <DealCard
              ...
              onToggle={() =>
                setExpandedDealId(expandedDealId === deal._id ? null : deal._id)
              }
            />
          ))
        )}
...
function BoardView({
  ...
  const pendingReg = partnerDeals.open.filter(
    (d: any) => d.registrationStatus === "pending",
  );
  const activePipeline = partnerDeals.open.filter(
    (d: any) => d.registrationStatus !== "pending",
  );
...
function GridView({
  ...
  return (
    <>
            ...
            >
              {partnerDeals.open.map((deal: any) => (
                <DealCard
                  ...
                  onToggle={() =>
                    setExpandedDealId(
                      expandedDealId === deal._id ? null : deal._id,
                    )
                  }
                />
              ))}
      ...
      {/* Won Deals */}
      {(filterStatus === "all" || filterStatus === "won") &&
      ...
      {/* Lost Deals */}
      {(filterStatus === "all" || filterStatus === "lost") &&
...
export default function PipelinePage() {
  ...
  const { open, won, lost, total, lostValue } = pipelineData;

  // Filter to only show deals with partner involvement
  const partnerDeals = {
    open: open.filter(
      (d: any) => d.involvedPartners && d.involvedPartners.length > 0,
    ),
    won: won.filter(
      (d: any) => d.involvedPartners && d.involvedPartners.length > 0,
    ),
  ...
  const hasPartnerDeals =
    partnerDeals.open.length > 0 ||
    partnerDeals.won.length > 0 ||
    partnerDeals.lost.length > 0;

  // Calculate partner-influenced metrics
  const partnerOpenValue = partnerDeals.open.reduce(
    (s: number, d: any) => s + d.amount,
    0,
  );
  const partnerWonValue = partnerDeals.won.reduce(
    (s: number, d: any) => s + d.amount,
    0,
  );
  const partnerTotalDeals =
    partnerDeals.open.length +
    partnerDeals.won.length +
    partnerDeals.lost.length;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      >
        <div>
          ...
          <p className="muted" style={{ marginTop: "0.25rem" }}>
            Partner-influenced deals and revenue pipeline
          </p>
        </div>
      ...
      {viewMode === "grid" && hasPartnerDeals && (
        <div className="card" style={{ padding: "1.25rem" }}>
          <div
            style={{ fontSize: ".875rem", fontWeight: 700, marginBottom: 12 }}
          >
            Pipeline by Stage
          </div>
          <div
          ...
          >
            <span style={{ color: "#6366f1", fontWeight: 600 }}>
              {formatCurrency(partnerOpenValue)} open
            </span>
            <span style={{ color: "#22c55e", fontWeight: 600 }}>
              {formatCurrency(partnerWonValue)} won
            </span>
            <span style={{ color: "#ef4444", fontWeight: 600 }}>
              {formatCurrency(lostValue)} lost
            </span>
          </div>
        </div>
      )}
