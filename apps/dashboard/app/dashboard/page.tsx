...
function useDeferredQuery(enabled: boolean) {
...
const FALLBACK_PIPELINE = [
  180, 160, 200, 220, 195, 210, 240, 230, 250, 215, 270, 290,
];
...
function calcTrendDelta(data: number[]): {
  pct: number;
...
function TrendBadge({ data }: { data: number[] }) {
  ...
  if (direction === "flat")
    return (
      <span
        style={{ fontSize: ".7rem", color: "var(--muted)", fontWeight: 500 }}
      >
        — vs last month
      </span>
    );
...
function WelcomeBanner() {
  ...
  const [configData, setConfigData] = useState<{
    programType: string;
    interactionTypes: { id: string }[];
    attributionModel: string;
    commissionRules: { label: string }[];
  } | null>(null);
  ...
  return (
    ...
    >
      <p style={{ fontSize: ".85rem", color: "var(--fg)" }}>
        <strong>Welcome!</strong> Your {configData.programType} program is
        configured. {configData.interactionTypes.length} interaction type
        {configData.interactionTypes.length !== 1 ? "s" : ""} ·{" "}
        {configData.attributionModel.replace(/_/g, " ")} attribution
        {configData.commissionRules.length > 0
          ? ` · ${configData.commissionRules[0].label}`
          : ""}
      </p>
      <button
        onClick={() => {
          setVisible(false);
          localStorage.setItem("covant_welcome_dismissed", "true");
        }}
        style={{
          background: "none",
          border: "none",
  ...
  return (
    ...
    >
      <p style={{ fontSize: ".85rem", color: "var(--fg)", margin: 0 }}>
        👋 You&apos;re in the Covant demo. Want to see it from a partner&apos;s
        perspective?{" "}
        <Link
          href="/portal"
          style={{
...
export default function DashboardPage() {
  ...
  const recentDeals = (dashboardData?.recentDeals ??
    [...storeDeals]
      ...
      .slice(0, 5)) as unknown as Deal[];
  const topPartners = (dashboardData?.topPartners ??
    storePartners
      .filter((p) => p.status === "active")
      .slice(0, 5)) as unknown as Partner[];
  const pendingPayouts = (dashboardData?.pendingPayouts ??
    storePayouts.filter(
      (p) => p.status === "pending_approval",
    )) as unknown as (Payout & { partner?: Partner })[];
  const auditLog = (dashboardData?.auditLog ??
    storeAuditLog.slice(0, 5)) as unknown as AuditEntry[];
  ...
  const pendingMDF = (convexMdfRequests ?? []).filter(
  ...
  const isEmpty =
    ...
    dashboardData.stats.totalPartners === 0 &&
    ...
    <>
      ...
      {isEmpty && !seedDone && (
        ...
        >
          ...
          >
            Load a sample dataset to explore the full platform — 5 partners, 17
            deals, attributions, and payouts already wired up.
          </p>
          <div
            style={{
              display: "flex",
      ...
      >
        <div>
          ...
          <p className="muted">
            Partner-influenced revenue &amp; program overview
          </p>
        </div>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: ".75rem" }}>
            ...
            <div>
              ...
              >
                This dashboard adapts to your workflow
              </p>
              <p style={{ fontSize: ".8rem", color: "#4338ca" }}>
                Toggle features, adjust complexity, and enable only what you
                need in Platform Configuration.
              </p>
            </div>
          </div>
          <Link
      ...
      {isFeatureEnabled("channelConflict") && openConflicts.length > 0 && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: ".75rem" }}>
            ...
            <div>
              <p
                style={{ fontWeight: 700, fontSize: ".9rem", color: "#991b1b" }}
              >
                {openConflicts.length} unresolved channel conflict
                {openConflicts.length !== 1 ? "s" : ""}
              </p>
              <p style={{ fontSize: ".8rem", color: "#b91c1c" }}>
                Multiple partners claiming the same accounts
              </p>
            </div>
          </div>
          <Link
      ...
      {isFeatureEnabled("mdf") && pendingMDF.length > 0 && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: ".75rem" }}>
            ...
            <div>
              <p
                style={{ fontWeight: 700, fontSize: ".9rem", color: "#78350f" }}
              >
                {pendingMDF.length} MDF request
                {pendingMDF.length !== 1 ? "s" : ""} awaiting approval
              </p>
              <p style={{ fontSize: ".8rem", color: "#92400e" }}>
                Partner marketing campaigns need review
              </p>
            </div>
          </div>
          <Link
      ...
      >
        ...
        >
          {[
            {
              label: "Tier reviews pending",
              value: actionItems ? String(actionItems.tierReviewsPending) : "—",
            ...
            },
            {
              ...
              value: actionItems ? String(actionItems.partnersOnboarding) : "—",
            ...
            },
            {
              ...
              value: actionItems
                ? formatCurrencyCompact(actionItems.unpaidCommissions)
                : "—",
            ...
            {
              label: "Pending deal regs",
              value: actionItems ? String(actionItems.pendingDealRegs) : "—",
              href: "/dashboard/deals",
              color: "#eab308",
              icon: "📋",
            },
            {
              label: "Email triggers active",
              value: actionItems
                ? `${actionItems.emailTriggersActive}/${actionItems.emailTriggersTotal}`
                : "—",
            ...
            },
            {
              ...
              value: actionItems ? String(actionItems.pendingInvites) : "—",
              href: "/dashboard/partners",
              color: "#22c55e",
              icon: "✉️",
            },
          ...
          >
            ...
            >
              <Activity
                size={16}
                color={
                  programHealth.overall >= 70
                    ? "#22c55e"
                    : programHealth.overall >= 40
                      ? "#eab308"
                      : "#ef4444"
                }
              />
      ...
      {convexTopRecommended &&
        convexTopRecommended.filter((r) => r.wonCount > 0 || r.totalRevenue > 0)
          .length > 0 && (
          <div
            className="card"
            style={{ padding: "1.25rem", marginBottom: "1.5rem" }}
          >
            ...
            >
              {convexTopRecommended
                .filter((r) => r.wonCount > 0 || r.totalRevenue > 0)
                .slice(0, 3)
                .map((r) => (
                  <Link
                    key={r.partner._id}
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        background:
                          r.recommendationScore >= 0.7
                            ? "#dcfce7"
                            : r.recommendationScore >= 0.4
                              ? "#dbeafe"
                              : "#fef9c3",
                        color:
                          r.recommendationScore >= 0.7
      ...
      <div className="stat-grid" style={{ marginBottom: "2rem" }}>
        <div className="card">
          ...
          >
            <Sparkline
              data={trends?.revenue ?? FALLBACK_REVENUE}
              color="#10b981"
            />
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <p style={{ fontSize: ".8rem", color: "#065f46" }}>
                ↑ {stats?.wonDeals ?? 0} won deals
              </p>
        ...
        <div className="card">
          ...
          >
            <div>
              <p
                ...
                style={{ fontSize: ".8rem", marginBottom: ".3rem" }}
              >
                Partner-Touched Pipeline
              </p>
              <p style={{ fontSize: "1.8rem", fontWeight: 800 }}>
                {formatCurrencyCompact(stats?.pipelineValue ?? 0)}
              </p>
            </div>
            <div
              style={{
            ...
            >
              <Briefcase size={18} color="#3730a3" />
            </div>
          </div>
          <div
            style={{
          ...
          >
            <Sparkline
              data={trends?.pipeline ?? FALLBACK_PIPELINE}
              color="#6366f1"
            />
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <p style={{ fontSize: ".8rem", color: "#3730a3" }}>
                {stats?.openDeals ?? 0} active deals
              </p>
        ...
        <div className="card">
          ...
          >
            <Sparkline
              data={trends?.partners ?? FALLBACK_PARTNERS}
              color="#22c55e"
            />
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <div className="card">
          ...
          >
            <Sparkline
              data={trends?.winRate ?? FALLBACK_WINRATE}
              color="#f59e0b"
            />
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <p style={{ fontSize: ".8rem", color: "#92400e" }}>
                Avg deal: {formatCurrencyCompact(stats.avgDealSize)}
              </p>
            ...
            recentDeals.map((deal) => (
              ...
              >
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <span
                    className={`badge badge-${deal.status === "won" ? "success" : deal.status === "lost" ? "danger" : "info"}`}
                  >
                    {deal.status}
                  </span>
                </div>
